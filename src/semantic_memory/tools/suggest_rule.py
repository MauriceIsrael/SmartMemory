...
SUGGEST_RULE_TOOL = Tool(
    name="suggest_rule",
    description=(
        "Propose a SPARQL inference rule to formalize a reasoning pattern discovered through conversation."
        "\n\n**WHEN TO USE:**"
        ...
        "\n\n**WORKFLOW:**"
        "\n1. User: 'Bob est un ami' (Bob is a friend)"
        "\n2. You try: add_memory(':User foaf:friend :Bob')"
        "\n3. Fails: foaf:friend not in vocabulary"
        "\n4. **YOU PROPOSE:** suggest_rule("
        "\n     rule_id='friendship_mutual',"
        "\n     description='Friendship is symmetric and implies knowing each other',"
        "\n     sparql_pattern='CONSTRUCT { ?b :isFriendOf ?a . ?a foaf:knows ?b . ?b foaf:knows ?a } WHERE { ?a :isFriendOf ?b }'"
        "\n   )"
        "\n5. System: Previews inferences, adds to pending approval"
        "\n6. User approves: approve_rule('friendship_mutual')"
        "\n7. Rule activates and infers facts"
        "\n\n**WORKFLOW 2 (Proactive pattern detection):**"
        "\n1. User asks: 'Can Gilles vote?'"
        "\n2. You query: No direct answer in graph"
        "\n3. You reason: 'Voting requires age ≥18. I don't have age, but Gilles has driving license'"
        "\n4. **YOU IMMEDIATELY PROPOSE:** suggest_rule("
        "\n     rule_id='driving_license_implies_adult',"
        "\n     description='Having a driving license implies being an adult (≥18)',"
        "\n     sparql_pattern='CONSTRUCT { ?person :isAdult true } WHERE { ?person :hasDrivingLicense ?license }'"
        "\n   )"
        "\n5. User approves → formal inference chain established"
        "\n6. Now you can answer: 'Yes, Gilles can vote (adult via driver's license)'"
        ...
        "\n\nBypassing this breaks the human-in-the-loop guarantees that make the system trustworthy!"
        "\n\n**Best Practices:**"
...
"""
        ...
        from semantic_memory.tools.pending_rules import add_pending_rule
